...
for (iyear in years) {
  ...
  if (iyear >= 2015) {
    download.file("http://cpw.state.co.us/Documents/RulesRegs/Brochure/BigGame/biggame.pdf",
                  paste(iyear,"COBigGame",sep=""))
  } else {
  ...
  #2017 http://www.eregulations.com/wp-content/uploads/2017/08/biggame.pdf
  #2016 http://www.craig-chamber.com/documents/2016biggamebrochure.pdf
  #2015 http://huntodds.monstermuleys.com/colorado/2015_colorado_big_game_regs.pdf
  
  
  # This function will directly export the raw text in a character vector with spaces to show 
  # the white space and \n to show the line breaks.
  COBigGame <- pdf_text(paste(iyear,"COBigGame",sep=""))
  ...
  COBigGame <- strsplit(COBigGame, "\n")
  
  # which pages have "ELK" in the first row?
  lapply( COBigGame , "[[" , 1)
  ElkPages <- grep("ELK",  lapply( COBigGame , "[[" , 1))
  COBigGame1 <- COBigGame[ElkPages]
  # unlist page elements
  COBigGame1 <- unlist(COBigGame1)
  # look for Rifle only
  RifleRows <- grep("RIFLE — LIMITED LICENSES", COBigGame1)
  # remove rows preceeding
  COBigGame2 <- COBigGame1[-1:-RifleRows[1]]
  COBigGame2 <- str_trim(COBigGame2,side = "left")
  COBigGame3 <- gsub(pattern = "\\s{2,}",replacement = ":|:",x = COBigGame2) # two or more white spaces
