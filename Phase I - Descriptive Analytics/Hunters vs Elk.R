...
ggplot(COElk.Hunters.Population.Statewide, aes(as.numeric(Year),Population/Hunters)) +
  geom_point(size=2) +
  geom_smooth(span=.75,se=F,size=3) +
  xlab("Year") +
  labs(title="Statewide Ratio of Elk to Hunters",subtitle="by Year", caption="source: cpw.state.co.us")

#' Statewide there are three distinct trents of the Elk population to Hunters ratio. The could be the result
...
for (imap in unique(COElkHuntersPopulation$Year)){
  png(file=paste("ElkperHunterMap",imap,".png"), width=1400, height=1000)
...
#' Elk to Hunter ratio is a good indication of how 'busy' it will be out in the forest though.